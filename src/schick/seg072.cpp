...
void INF_eliane_tiomar(const signed int informer, const signed int state)
{
	if (!informer) {
		...
		if (!state) {
			...
			/* drink with TIOMAR */
			timewarp(HOURS(1));
			g_dialog_next_state = (test_talent(get_hero((gs_tiomar_drinkmate = (unsigned char)get_random_hero())), TA_ZECHEN, 0) > 0 ? 21 : 22);
		} else if (state == 22) {
...
void INF_swafnild_unicorn(const signed int informer, const signed int state)
{
	if (!informer) {
		...
		if (state == 1) {
			...
			gs_town_id = (gs_swafnild_destination == 1 ? gs_swafnild_tp1 :
